...
const Tabs: React.FC<TabsProps> = ({
  ...
  return (
    <div
      className={`flex justify-between hero-card border-[1px] w-full border-stroke backdrop-blur-xl py-6 px-3 lg:px-7 ${isLoading ? 'rounded-b-lg' : 'rounded-lg'}`}
    >
      {tabs.map((tab) => (
        <button
          key={tab.id}
          className={`flex items-center gap-2 md:gap-5 py-2 px-6 text-[10px] md:text-sm font-medium cursor-pointer transition-all duration-100 ${
            activeTab === tab.id ? 'border-b-2 border-primary text-primary' : 'text-white'
          }`}
          onClick={() => {
            setIsLoading(false);
            setActiveTab(tab.id);
